...
intComSpeedFLARM = 19200 # 19200
intComPinFLARM_RX = 18
intComPinFLARM_TX = 22

intComSpeedBUTTERFLY = 19200 # 57600
...
def subGeoCalc(LatMe,LongMe,LatTarget,LongTarget):
        ...
        if skipGeo == 0 and (len(LatTarget) > 0 or len(LongTarget) > 0):
                ...
                #subWrite2LogsubWrite2Log("LatMe: " + str(LatMe) + "LongMe: " + str(LongMe) + " Bearing N : " + str(bearingN) + " Bearing: " + str(bearing) $
...
def subExtractADSBInfo(Sentence,MyLat,MyLong):
        ...
        #print "ADSB ICAO: " + ICAO + " Flags: " + Flags + "Call: " + Call + " tempCall: " + tempCall + " Squak: " + Squak + " Lat: " + Lat + " Long: " + Lo$

        #if Distance == 0 or Bearing == 0 or (Distance/ZoomFactor) > MaxRange or Call == MyID or str(VelH) == Empty or str(VelV) == Empty or deltaAlt > MaxD$
        if Distance == 0 or Bearing == 0 or (Distance/ZoomFactor) > MaxRange or strADSBSplit.ICAO == MyID or str(strADSBSplit.VelH) == Empty or str(strADSBS$
                # Distance and bearing is zero, values are not valid OR the plane is too far away OR its my own signal OR VelV and VelH are empty: skip furt$
                Whatever = 1
                ...
                # PFLAA-message
                strPFLAAMsg =  "$PFLAA,0," + str(int(deltaNorth)) + "," + str(int(deltaEast)) + "," + str(int(deltaAlt)) + ",1," + strADSBSplit.ICAO + "," +$
                #listPlanes.append("PFLAA,0," + str(int(deltaNorth)) + "," + str(int(deltaEast)) + "," + str(int(deltaAlt)) + ",1," + tempCall + "," + Track$
...
def subExtractNMEAInfo(Sentence):
        ...
        #subWrite2Log("NMEA Time: " + strNMEASplit.Time + " , RW: " + strNMEASplit.RecieverWarning + " , lat: " + strNMEASplit.Lat +  " , long: " + strNMEAS$
        ...
        try:
                ...
                if (len(strNMEASplit.Lat) > 0 or len(strNMEASplit.Long) > 0):
                        ...
                        # If the string is empty, make it 0 (zero)
                        if LatDegrees == '':
                                LatDegrees = '0'
...
while go == 1:
        ...
        # A "S"-sentence has arrived. It is sent once every 1 second and this is used to trigger some functions:  time to send information to the butterfly $
        ...
        if str(strADSB).startswith("#S"):
                ...
                intSec = int(MyTime[4:])
                TimeStamp = intHrs * 60 * 60 * 1000 + intMin * 60 * 1000 + intSec * 1000
                #subSendSentence("$PAAVS,AT,ABS," + str(TimeStamp) + "," + str(NoOfFlarmContacts) + "," + str(NoOfFlarmContacts) + "," + str(MyLat) + "," + $
